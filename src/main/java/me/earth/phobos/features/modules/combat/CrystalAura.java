...
public class CrystalAura
    ...
    public CrystalAura() {
        super("CrystalAura", "Original phobos 1.9.0 ca no edits cause mfs cant config optimized one.", Module.Category.COMBAT, true, false, false);
        instance = this;
    }
    ...
    public void onPacketSend(PacketEvent.Send event) {
        ...
        BlockPos pos = null;
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (packet = event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet.getEntityFromWorld(CrystalAura.mc.world) instanceof EntityEnderCrystal) {
            pos = packet.getEntityFromWorld(CrystalAura.mc.world).getPosition();
            if (this.removeAfterAttack.getValue().booleanValue()) {
                Objects.requireNonNull(packet.getEntityFromWorld(CrystalAura.mc.world)).setDead();
                CrystalAura.mc.world.removeEntityFromWorld(packet.entityId);
            }
        }
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (packet = event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet.getEntityFromWorld(CrystalAura.mc.world) instanceof EntityEnderCrystal) {
            EntityEnderCrystal crystal = (EntityEnderCrystal) packet.getEntityFromWorld(CrystalAura.mc.world);
        ...
        if (!this.justRender.getValue().booleanValue() && this.switchTimer.passedMs(this.switchCooldown.getValue().intValue()) && this.explode.getValue().booleanValue() && this.instant.getValue().booleanValue() && event.getPacket() instanceof SPacketSpawnObject && (this.syncedCrystalPos == null || !this.syncedFeetPlace.getValue().booleanValue() || this.damageSync.getValue() == DamageSync.NONE)) {
            ...
            if (packet2.getType() == 51 && CrystalAura.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + (double) this.predictOffset.getValue().floatValue() <= MathUtil.square(this.breakRange.getValue().floatValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue().intValue()) || this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue().intValue()))) {
                ...
                if (this.predictFriendDmg.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (EntityPlayer friend : CrystalAura.mc.world.playerEntities) {
                        if (friend == null || CrystalAura.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtil.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !Phobos.friendManager.isFriend(friend) || !((double) DamageUtil.calculateDamage(pos, friend) > (double) EntityUtil.getHealth(friend) + 0.5))
                            continue;
                        return;
                    }
                }
                if (placedPos.contains(pos.down())) {
                    float selfDamage;
                    if (this.isRightThread() && this.superSafe.getValue() != false ? DamageUtil.canTakeDamage(this.suicide.getValue()) && ((double) (selfDamage = DamageUtil.calculateDamage(pos, CrystalAura.mc.player)) - 0.5 > (double) EntityUtil.getHealth(CrystalAura.mc.player) || selfDamage > this.maxSelfBreak.getValue().floatValue()) : this.superSafe.getValue() != false) {
                        return;
                    }
                    ...
                    if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
                        selfDamage = DamageUtil.calculateDamage(pos, CrystalAura.mc.player);
                    }
                    if ((double) selfDamage + 0.5 < (double) EntityUtil.getHealth(CrystalAura.mc.player) && selfDamage <= this.maxSelfBreak.getValue().floatValue()) {
                        for (EntityPlayer player : CrystalAura.mc.world.playerEntities) {
                            float damage;
            ...
            for (int id : packet4.getEntityIDs()) {
                Entity entity = CrystalAura.mc.world.getEntityByID(id);
            ...
            if (packet5.getOpCode() == 35 && packet5.getEntity(CrystalAura.mc.world) instanceof EntityPlayer) {
                this.totemPops.put((EntityPlayer) packet5.getEntity(CrystalAura.mc.world), new Timer().reset());
            }
            ...
            if (this.threadMode.getValue() == ThreadMode.SOUND && this.isRightThread() && CrystalAura.mc.player != null && CrystalAura.mc.player.getDistanceSq(pos) < MathUtil.square(this.soundPlayer.getValue().floatValue())) {
    ...
    private void attackCrystalPredict(int entityID, BlockPos pos) {
        ...
        CrystalAura.mc.player.connection.sendPacket(attackPacket);
        if (this.breakSwing.getValue().booleanValue()) {
            CrystalAura.mc.player.connection.sendPacket(new CPacketAnimation(EnumHand.MAIN_HAND));
        }
    ...
    private void postProcessBreak() {
        while (!this.packetUseEntities.isEmpty()) {
            ...
            CrystalAura.mc.player.connection.sendPacket(packet);
            if (this.breakSwing.getValue().booleanValue()) {
    ...
    private void handleWhile() {
        if (this.thread == null || this.thread.isInterrupted() || !this.thread.isAlive() || this.syncroTimer.passedMs(this.syncThreads.getValue().intValue()) && this.syncThreadBool.getValue().booleanValue()) {
            if (this.thread == null) {
                this.thread = new Thread(RCrystalAura.getInstance(this));
    ...
    private ScheduledExecutorService getExecutor() {
        ...
        service.scheduleAtFixedRate(RCrystalAura.getInstance(this), 0L, this.threadDelay.getValue().intValue(), TimeUnit.MILLISECONDS);
        return service;
    }
    ...
    private boolean check() {
        ...
        if (!((this.offHand || this.mainHand || this.switchMode.getValue() != Switch.BREAKSLOT || this.switching) && DamageUtil.canBreakWeakness(CrystalAura.mc.player) && this.switchTimer.passedMs(this.switchCooldown.getValue().intValue()))) {
            this.renderPos = null;
            target = null;
            this.rotating = false;
            return false;
        }
        if (this.mineSwitch.getValue().booleanValue() && Mouse.isButtonDown(0) && (this.switching || this.autoSwitch.getValue() == AutoSwitch.ALWAYS) && Mouse.isButtonDown(1) && CrystalAura.mc.player.getHeldItemMainhand().getItem() instanceof ItemPickaxe) {
            this.switchItem();
        }
        this.mapCrystals();
    ...
    private void mapCrystals() {
        ...
        for (Entity entity : CrystalAura.mc.world.loadedEntityList) {
            ...
            float selfDamage = -1.0f;
            if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
                selfDamage = DamageUtil.calculateDamage(entity, CrystalAura.mc.player);
            }
            if ((double) selfDamage + 0.5 < (double) EntityUtil.getHealth(CrystalAura.mc.player) && selfDamage <= this.maxSelfBreak.getValue().floatValue()) {
        ...
        if (this.webAttack.getValue().booleanValue() && this.webPos != null) {
            if (CrystalAura.mc.player.getDistanceSq(this.webPos.up()) > MathUtil.square(this.breakRange.getValue().floatValue())) {
                this.webPos = null;
            } else {
                for (Entity entity : CrystalAura.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(this.webPos.up()))) {
                    if (!(entity instanceof EntityEnderCrystal)) continue;
    ...
    private void calculateDamage(EntityPlayer targettedPlayer) {
        ...
        if (this.webAttack.getValue().booleanValue() && targettedPlayer != null && (web = CrystalAura.mc.world.getBlockState(playerPos = new BlockPos(targettedPlayer.getPositionVector())).getBlock()) == Blocks.WEB) {
            setToAir = playerPos;
        ...
        for (BlockPos pos : BlockUtil.possiblePlacePositions(this.placeRange.getValue().floatValue(), this.antiSurround.getValue(), this.oneDot15.getValue())) {
            if (!BlockUtil.rayTracePlaceCheck(pos, (this.raytrace.getValue() == Raytrace.PLACE || this.raytrace.getValue() == Raytrace.FULL) && CrystalAura.mc.player.getDistanceSq(pos) > MathUtil.square(this.placetrace.getValue().floatValue()), 1.0f))
                continue;
            float selfDamage = -1.0f;
            if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
                selfDamage = DamageUtil.calculateDamage(pos, CrystalAura.mc.player);
            }
            if (!((double) selfDamage + 0.5 < (double) EntityUtil.getHealth(CrystalAura.mc.player)) || !(selfDamage <= this.maxSelfPlace.getValue().floatValue()))
                continue;
            if (targettedPlayer != null) {
                ...
                if (this.calcEvenIfNoDamage.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.ALL || this.antiFriendPop.getValue() == AntiFriendPop.PLACE)) {
                    ...
                    for (EntityPlayer friend : CrystalAura.mc.world.playerEntities) {
                        float friendDamage;
                        if (friend == null || CrystalAura.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtil.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !Phobos.friendManager.isFriend(friend) || !((double) (friendDamage = DamageUtil.calculateDamage(pos, friend)) > (double) EntityUtil.getHealth(friend) + 0.5))
                            continue;
                        friendPop = true;
                        break;
                    }
    ...
    private EntityPlayer getTarget(boolean unsafe) {
        ...
        for (EntityPlayer player : CrystalAura.mc.world.playerEntities) {
            ...
            if (!(CrystalAura.mc.player.getDistanceSq(player) < CrystalAura.mc.player.getDistanceSq(currentTarget)))
                continue;
            currentTarget = player;
        }
        ...
        if (this.predictPos.getValue().booleanValue() && currentTarget != null) {
            ...
            EntityOtherPlayerMP newTarget = new EntityOtherPlayerMP(CrystalAura.mc.world, profile);
    ...
    private void doFakeSwing() {
        if (this.fakeSwing.getValue().booleanValue()) {
            EntityUtil.swingArmNoPacket(EnumHand.MAIN_HAND, CrystalAura.mc.player);
        }
    }
    ...
    private void manualBreaker() {
        ...
        if ((this.offHand || this.mainHand) && this.manual.getValue().booleanValue() && this.manualTimer.passedMs(this.manualBreak.getValue().intValue()) && Mouse.isButtonDown(1) && CrystalAura.mc.player.getHeldItemOffhand().getItem() != Items.GOLDEN_APPLE && CrystalAura.mc.player.inventory.getCurrentItem().getItem() != Items.GOLDEN_APPLE && CrystalAura.mc.player.inventory.getCurrentItem().getItem() != Items.BOW && CrystalAura.mc.player.inventory.getCurrentItem().getItem() != Items.EXPERIENCE_BOTTLE && (result = CrystalAura.mc.objectMouseOver) != null) {
            switch (result.typeOfHit) {
                ...
                case BLOCK: {
                    BlockPos mousePos = CrystalAura.mc.objectMouseOver.getBlockPos().up();
    ...
    private boolean isValid(Entity entity) {
        return entity != null && CrystalAura.mc.player.getDistanceSq(entity) <= MathUtil.square(this.breakRange.getValue().floatValue()) && (this.raytrace.getValue() == Raytrace.NONE || this.raytrace.getValue() == Raytrace.PLACE || CrystalAura.mc.player.canEntityBeSeen(entity) || !CrystalAura.mc.player.canEntityBeSeen(entity) && CrystalAura.mc.player.getDistanceSq(entity) <= MathUtil.square(this.breaktrace.getValue().floatValue()));
    }
    ...
    private static class RCrystalAura
            implements Runnable {
        private static RCrystalAura instance;
        private CrystalAura CrystalAura;

        private RCrystalAura() {
        }

        public static RCrystalAura getInstance(CrystalAura CrystalAura) {
            if (instance == null) {
                instance = new RCrystalAura();
                RCrystalAura.instance.CrystalAura = CrystalAura;
            }
            return instance;
        }
        ...
        @Override
        public void run() {
            if (this.CrystalAura.threadMode.getValue() == ThreadMode.WHILE) {
                while (this.CrystalAura.isOn() && this.CrystalAura.threadMode.getValue() == ThreadMode.WHILE) {
                    while (Phobos.eventManager.ticksOngoing()) {
                    }
                    if (this.CrystalAura.shouldInterrupt.get()) {
                        this.CrystalAura.shouldInterrupt.set(false);
                        this.CrystalAura.syncroTimer.reset();
                        this.CrystalAura.thread.interrupt();
                        break;
                    }
                    this.CrystalAura.threadOngoing.set(true);
                    Phobos.safetyManager.doSafetyCheck();
                    this.CrystalAura.doCrystalAura();
                    this.CrystalAura.threadOngoing.set(false);
                    try {
                        Thread.sleep(this.CrystalAura.threadDelay.getValue().intValue());
                    } catch (InterruptedException e) {
                        this.CrystalAura.thread.interrupt();
                        e.printStackTrace();
                    }
                }
            } else if (this.CrystalAura.threadMode.getValue() != ThreadMode.NONE && this.CrystalAura.isOn()) {
                while (Phobos.eventManager.ticksOngoing()) {
                }
                this.CrystalAura.threadOngoing.set(true);
                Phobos.safetyManager.doSafetyCheck();
